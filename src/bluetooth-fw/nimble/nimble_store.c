...
static int prv_nimble_store_write_peer_sec(const struct ble_store_value_sec *value_sec) {
  ...
  if (value_sec->key_size != KEY_SIZE) {
    PBL_LOG_D(LOG_DOMAIN_BT, LOG_LEVEL_ERROR, "Unsupported key size %d", value_sec->key_size);
...
void bt_driver_handle_host_added_bonding(const BleBonding *bonding) {
  ...
  s->value_sec.authenticated = !!(bonding->flags & BLE_FLAG_AUTHENTICATED);
