...
ble_store_pebble_write_peer_sec(const struct ble_store_value_sec *value_sec)
{
    ...
    if (value_sec->irk_present) {
        BleIRKChange irk_change_event = { 0 };

        irk_change_event.irk_valid = true,
        memcpy(irk_change_event.irk.data, value_sec->irk, 16);

        irk_change_event.device.is_random_address = value_sec->peer_addr.type == BLE_ADDR_RANDOM;
        memcpy(irk_change_event.device.address.octets, &value_sec->peer_addr.val, 6);
